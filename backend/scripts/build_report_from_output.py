...
def status_color(s: str) -> colors.Color:
    return {"PASSED": GREEN, "FAILED": RED, "ERROR": RED,
            "SKIPPED": YELLOW, "XFAIL": YELLOW}.get(s, GRAY)


def status_label(s: str) -> str:
    return {"PASSED": "✓  PASSÉ", "FAILED": "✗  ÉCHOUÉ", "ERROR": "✗  ERREUR",
            "SKIPPED": "○  IGNORÉ", "XFAIL": "○  XFAIL"}.get(s, s)
...
def build_pdf(data: dict, out_path: str) -> None:
    ...
    title_box = Table([[
        _p("<b>RAPPORT DE TESTS</b>",
           fontSize=16, fontName="Helvetica-Bold", textColor=NAVY, alignment=2),
    ]], colWidths=[half])
    ...
    s.append(_p(
        ...
        fontSize=8, fontName="Helvetica", textColor=BLACK, alignment=2,
    ))
    ...
    def _kpi(val: str, label: str, vc=NAVY):
        t = Table([[
            _p(f"<b>{val}</b>", fontSize=24, fontName="Helvetica-Bold",
               textColor=vc, alignment=1),
        ], [
            _p(label, fontSize=7.5, fontName="Helvetica", textColor=GRAY, alignment=1),
        ]], colWidths=[w4])
    ...
    if data["failed"]:
        note = Table([[
            _p(f"<b>Note sur les {data['failed']} échecs</b>",
               fontSize=8.5, fontName="Helvetica-Bold", textColor=YELLOW),
        ], [
            _p(FAILURE_ROOT_CAUSE,
               fontSize=7.5, fontName="Helvetica", textColor=BLACK, leading=11),
        ]], colWidths=[cw])
    ...
    # ── Résumé par module ────────────────────────────────────────────────────
    s.append(_p("<b>Résumé par module</b>",
    ...
    hdr = [[
        ...
        _p("Total",  fontSize=7.5, fontName="Helvetica-Bold", textColor=BLACK, alignment=1),
        _p("Passés", fontSize=7.5, fontName="Helvetica-Bold", textColor=BLACK, alignment=1),
        _p("Échoués",fontSize=7.5, fontName="Helvetica-Bold", textColor=BLACK, alignment=1),
        _p("Statut", fontSize=7.5, fontName="Helvetica-Bold", textColor=BLACK, alignment=1),
    ]]
    mod_style = [
    ...
    for i, (mod, tests) in enumerate(sorted(data["by_module"].items()), 1):
        ...
        rows.append([
            ...
            _p(str(tot), fontSize=7.5, fontName="Helvetica", textColor=GRAY, alignment=1),
            _p(str(ok),  fontSize=7.5, fontName="Helvetica", textColor=GREEN, alignment=1),
            _p(str(ko) if ko else "—",
               fontSize=7.5, fontName="Helvetica",
               textColor=RED if ko else GRAY, alignment=1),
            _p("✓" if not ko else f"✗ {ko}",
               fontSize=7.5, fontName="Helvetica-Bold",
               textColor=GREEN if not ko else RED, alignment=1),
        ])
    ...
    for mod, tests in sorted(data["by_module"].items()):
        ...
        s.append(_p(
            ...
            fontSize=9.5, fontName="Helvetica-Bold", textColor=NAVY,
        ))
        ...
        cn3 = cw * 0.15
        rows2 = [[
            _p("Nom du test", fontSize=7, fontName="Helvetica-Bold", textColor=BLACK),
            _p("Note",        fontSize=7, fontName="Helvetica-Bold", textColor=BLACK),
            _p("Statut",      fontSize=7, fontName="Helvetica-Bold", textColor=BLACK, alignment=2),
        ]]
        tstyle = [
            ("BACKGROUND",    (0, 0), (-1, 0),  LGRAY),
        ...
        for i, t in enumerate(tests, 1):
            ...
            rows2.append([
                ...
                _p(note, fontSize=6, fontName="Helvetica-Oblique", textColor=YELLOW if note else GRAY),
                _p(f"<b>{status_label(t['status'])}</b>",
                   fontSize=6.5, fontName="Helvetica-Bold",
                   textColor=status_color(t["status"]), alignment=2),
            ])
