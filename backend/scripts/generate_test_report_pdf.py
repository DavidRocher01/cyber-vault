...
def build_pdf(data: dict, out_path: str) -> None:
    ...
    s.append(_p(
        ...
        fontSize=8.5, fontName="Helvetica", textColor=BLACK, alignment=2,
    ))
    ...
    def _stat_cell(value: str, label: str, val_color=NAVY):
        return Table([[
            _p(f"<b>{value}</b>",
               fontSize=22, fontName="Helvetica-Bold", textColor=val_color, alignment=1),
        ], [
    ...
    stats_row = Table([[
        ...
        _stat_cell(str(data["failed"] + data["errors"]), "Échoués", RED if data["failed"] + data["errors"] else GRAY),
    ...
    summary_rows = [[
        ...
        _p("Total",  fontSize=8, fontName="Helvetica-Bold", textColor=BLACK, alignment=1),
        _p("Passés", fontSize=8, fontName="Helvetica-Bold", textColor=BLACK, alignment=1),
        _p("Échoués",fontSize=8, fontName="Helvetica-Bold", textColor=BLACK, alignment=1),
        _p("Statut", fontSize=8, fontName="Helvetica-Bold", textColor=BLACK, alignment=1),
    ]]
    col_mod = cw * 0.52
    col_num = cw * 0.12
    col_sta = cw * 0.12
    ...
    for i, (mod, tests) in enumerate(sorted(data["by_module"].items()), 1):
        ...
        summary_rows.append([
            ...
            _p(str(total_m),  fontSize=8, fontName="Helvetica", textColor=GRAY,  alignment=1),
            _p(str(passed_m), fontSize=8, fontName="Helvetica", textColor=GREEN, alignment=1),
            _p(str(failed_m) if failed_m else "—",
               fontSize=8, fontName="Helvetica",
               textColor=RED if failed_m else GRAY, alignment=1),
            _p("✓" if ok else f"✗ {failed_m}",
               fontSize=8, fontName="Helvetica-Bold",
               textColor=GREEN if ok else RED, alignment=1),
        ])
    ...
    for mod, tests in sorted(data["by_module"].items()):
        ...
        col_s = cw * 0.22
        rows  = [[
            _p("Nom du test", fontSize=7.5, fontName="Helvetica-Bold", textColor=BLACK),
            _p("Statut",      fontSize=7.5, fontName="Helvetica-Bold", textColor=BLACK, alignment=2),
        ]]
        tstyle = [
            ("BACKGROUND",    (0, 0), (-1, 0),  LGRAY),
...
if __name__ == "__main__":
    ...
    print(f"  {data['total']} tests — {data['passed']} passés, {data['failed'] + data['errors']} échoués")
