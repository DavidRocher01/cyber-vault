...
def get_commits(since: str | None) -> list[dict]:
    if since:
        ...
        if since.startswith("v") or re.match(r"^\d+\.\d+", since):
            cmd = ["git", "log", f"{since}..", "--pretty=format:%H%x00%s%x00%an%x00%ad", "--date=short"]
        else:
            cmd = ["git", "log", f"--since={since}", "--pretty=format:%H%x00%s%x00%an%x00%ad", "--date=short"]
    else:
        since_date = (datetime.utcnow() - timedelta(days=30)).strftime("%Y-%m-%d")
        cmd = ["git", "log", f"--since={since_date}", "--pretty=format:%H%x00%s%x00%an%x00%ad", "--date=short"]
    ...
    for line in result.stdout.strip().splitlines():
        ...
        if len(parts) == 4:
            commits.append({"hash": parts[0][:7], "subject": parts[1], "author": parts[2], "date": parts[3]})
    return commits
...
def build_pdf(groups: dict, tag: str, since: str | None, out_path: str) -> None:
    ...
    title_label = f"CHANGELOG — {tag}" if tag and tag != "" else f"CHANGELOG"
    title_box = Table([[
        _p(f"<b>{title_label}</b>",
           fontSize=14, fontName="Helvetica-Bold", textColor=NAVY, alignment=2),
    ]], colWidths=[half])
    ...
    if not groups:
        s.append(_p("Aucun commit trouvé pour cette période.",
        ...
        for typ_key in COMMIT_TYPES:
            ...
            header_tbl = Table([[
                ...
                _p(f"{len(commits)}",
                   fontSize=9, fontName="Helvetica-Bold", textColor=WHITE, alignment=2),
            ...
            for i, c in enumerate(commits):
                ...
                rows.append([
                    _p(f"<font color='#64748b'><b>{c['hash']}</b></font>",
                       fontSize=7, fontName="Helvetica-Bold", textColor=GRAY),
                    _p(c["message"], fontSize=7.5, fontName="Helvetica", textColor=BLACK),
                    _p(c["date"], fontSize=7, fontName="Helvetica", textColor=GRAY, alignment=2),
                ])
