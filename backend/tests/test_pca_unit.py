...
def _basic_payload() -> PcaRequest:
    return PcaRequest(
        ...
        critical_systems=[
            PcaSystem(name="ERP", description="Gestion des stocks", rto_hours=4, rpo_hours=1, responsible="Alice"),
        ],
        response_team=[
...
def test_pdf_is_non_empty_bytes():
    data = {"company": {"name": "Test", "sector": ""}, "critical_systems": [], "response_team": []}
...
def test_pdf_with_systems():
    data = {
        ...
        "critical_systems": [
            {"name": "CRM", "description": "Gestion clients", "rto_hours": 2, "rpo_hours": 1, "responsible": "Alice"},
            {"name": "ERP", "description": "Comptabilité", "rto_hours": 8, "rpo_hours": 4, "responsible": "Bob"},
        ],
        "response_team": [],
    }
    pdf = generate_pca_pdf(data)
