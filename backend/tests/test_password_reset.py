...
async def test_forgot_password_existing_email_returns_202():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(
            ...
            json={"email": "reset@test.com", "password": "StrongPass123!"},
        )
...
async def test_forgot_password_sends_email_for_valid_user():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(
            f"{BASE}/auth/register",
            json={"email": "sendmail@test.com", "password": "StrongPass123!"},
        )
...
async def test_reset_password_valid_token_returns_204():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(
            f"{BASE}/auth/register",
            json={"email": "r1@test.com", "password": "StrongPass123!"},
        )
...
async def test_reset_password_allows_login_with_new_password():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(
            f"{BASE}/auth/register",
            json={"email": "r2@test.com", "password": "StrongPass123!"},
        )
        ...
        login = await c.post(
            f"{BASE}/auth/login",
            json={"email": "r2@test.com", "password": "NewStrongPass456!"},
        )
...
async def test_reset_password_old_password_no_longer_works():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(
            f"{BASE}/auth/register",
            json={"email": "r3@test.com", "password": "StrongPass123!"},
        )
...
async def test_reset_password_used_token_returns_400():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(
            f"{BASE}/auth/register",
            json={"email": "r4@test.com", "password": "StrongPass123!"},
        )
        ...
        r2 = await c.post(
            f"{BASE}/auth/reset-password",
            json={"token": raw_token, "password": "AnotherPass789!"},
        )
...
async def test_reset_password_expired_token_returns_400():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(
            f"{BASE}/auth/register",
            json={"email": "r5@test.com", "password": "StrongPass123!"},
        )
...
async def test_reset_password_weak_password_rejected():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(
            f"{BASE}/auth/register",
            json={"email": "r6@test.com", "password": "StrongPass123!"},
        )
        ...
        r = await c.post(
            f"{BASE}/auth/reset-password", json={"token": raw_token, "password": "abc"}
        )
