...
async def _auth_consultant(http_client: AsyncClient, db_session, email: str) -> dict:
...
@pytest.mark.asyncio
async def test_profile_initial_values_are_null(http_client: AsyncClient, db_session):
    h = await _auth_consultant(http_client, db_session, "profile_init@test.com")
    r = await http_client.get(f"{BASE}/rssi/profile", headers=h)
...
@pytest.mark.asyncio
async def test_profile_patch_all_fields(http_client: AsyncClient, db_session):
    h = await _auth_consultant(http_client, db_session, "profile_patch@test.com")
...
@pytest.mark.asyncio
async def test_profile_patch_partial(http_client: AsyncClient, db_session):
    h = await _auth_consultant(http_client, db_session, "profile_partial@test.com")
...
@pytest.mark.asyncio
async def test_profile_patch_empty_string_clears_field(http_client: AsyncClient, db_session):
    h = await _auth_consultant(http_client, db_session, "profile_clear@test.com")
...
@pytest.mark.asyncio
async def test_profile_get_returns_email(http_client: AsyncClient, db_session):
    h = await _auth_consultant(http_client, db_session, "profile_email@test.com")
    r = await http_client.get(f"{BASE}/rssi/profile", headers=h)
...
async def test_csv_export_unknown_client_404(http_client: AsyncClient):
    h = await _auth(http_client, "csv_404@test.com")
...
async def test_csv_export_empty_has_header_only(http_client: AsyncClient):
    h = await _auth(http_client, "csv_empty@test.com")
...
async def test_csv_export_header_columns(http_client: AsyncClient):
    h = await _auth(http_client, "csv_hdr@test.com")
...
async def test_csv_export_contains_actions(http_client: AsyncClient):
    h = await _auth(http_client, "csv_data@test.com")
    c = await _create_client(http_client, h)
...
async def test_csv_export_cross_user_isolation(http_client: AsyncClient):
    h1 = await _auth(http_client, "csv_u1@test.com")
    h2 = await _auth(http_client, "csv_u2@test.com")
...
async def test_csv_export_uses_french_labels(http_client: AsyncClient):
    h = await _auth(http_client, "csv_fr@test.com")
    c = await _create_client(http_client, h)
