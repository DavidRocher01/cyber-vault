...
async def test_add_site_no_subscription_returns_403():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        ...
        r = await c.post(
            f"{BASE}/sites",
            json={"url": "https://mysite.com", "name": "My Site"},
            headers=h,
        )
...
async def test_add_site_with_mocked_quota():
    ...
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            r = await c.post(
                f"{BASE}/sites",
                json={"url": "https://mysite.com", "name": "Site A"},
                headers=h,
            )
...
async def test_add_site_url_gets_https_prefix():
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            r = await c.post(
                f"{BASE}/sites",
                json={"url": "example.com", "name": "No Scheme"},
                headers=h,
            )
    assert r.status_code == 201
    ...
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            created = await c.post(
                f"{BASE}/sites",
                json={"url": "https://del.com", "name": "Del"},
                headers=h,
            )
...
async def test_delete_other_user_site_returns_404():
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            created = await c.post(
                f"{BASE}/sites",
                json={"url": "https://private.com", "name": "X"},
                headers=h1,
            )
            site_id = created.json()["id"]
...
async def test_quota_exceeded_returns_403():
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            await c.post(
                f"{BASE}/sites",
                json={"url": "https://first.com", "name": "First"},
                headers=h,
            )
            r = await c.post(
                f"{BASE}/sites",
...
async def test_sites_not_visible_to_other_users():
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            await c.post(
                f"{BASE}/sites",
                json={"url": "https://secret.com", "name": "Secret"},
                headers=h1,
            )
...
async def test_add_site_ftp_protocol_returns_422():
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            r = await c.post(
                f"{BASE}/sites",
                json={"url": "ftp://example.com", "name": "FTP"},
                headers=h,
            )
...
async def test_add_site_javascript_protocol_returns_422():
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            r = await c.post(
                f"{BASE}/sites",
                json={"url": "javascript:alert(1)", "name": "XSS"},
                headers=h,
            )
    assert r.status_code == 422
...
async def test_add_site_data_protocol_returns_422():
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            r = await c.post(
                f"{BASE}/sites",
                json={"url": "data:text/html,<h1>x</h1>", "name": "Data"},
                headers=h,
            )
    assert r.status_code == 422
...
async def test_get_site_subdomains_no_scan_returns_empty():
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
            ...
            created = await c.post(
                f"{BASE}/sites",
                json={"url": "https://nosubdom.com", "name": "No scan"},
                headers=h,
            )
            site_id = created.json()["id"]
