...
@pytest.mark.parametrize("xp,expected_level,expected_label", [
    (0,   1, "Apprenti"),
    (50,  1, "Apprenti"),
    (51,  2, "Initié"),
    (150, 2, "Initié"),
...
async def test_get_my_level_returns_level_data():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(f"{BASE}/auth/register", json={"email": "gami_admin@test.com", "password": "StrongPass123!"})
        r = await c.post(f"{BASE}/auth/login", json={"email": "gami_admin@test.com", "password": "StrongPass123!"})
        admin_h = {"Authorization": f"Bearer {r.json()['access_token']}"}

        org_id = (
            await c.post(f"{BASE}/awareness/organizations", json={"name": "Gami Org"}, headers=admin_h)
        ).json()["id"]
        await c.post(
        ...
        token = (
            await c.post(
                ...
                json={"email": "gami_learner@test.com", "organization_id": org_id},
            )
        ).json()["token"]
...
async def test_get_my_badges_returns_empty_list_initially():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(f"{BASE}/auth/register", json={"email": "badge_admin@test.com", "password": "StrongPass123!"})
        r = await c.post(f"{BASE}/auth/login", json={"email": "badge_admin@test.com", "password": "StrongPass123!"})
        admin_h = {"Authorization": f"Bearer {r.json()['access_token']}"}

        org_id = (
            await c.post(f"{BASE}/awareness/organizations", json={"name": "Badge Org"}, headers=admin_h)
        ).json()["id"]
        await c.post(
            f"{BASE}/awareness/organizations/{org_id}/learners",
        ...
        token = (
            await c.post(
                f"{BASE}/awareness/auth/magic-link",
                json={"email": "badge_learner@test.com", "organization_id": org_id},
            )
        ).json()["token"]
        session = (await c.get(f"{BASE}/awareness/auth/verify", params={"token": token})).json()
        learner_h = {"Authorization": f"Bearer {session['access_token']}"}
...
async def test_leaderboard_returns_list():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as c:
        await c.post(f"{BASE}/auth/register", json={"email": "lb_admin@test.com", "password": "StrongPass123!"})
        r = await c.post(f"{BASE}/auth/login", json={"email": "lb_admin@test.com", "password": "StrongPass123!"})
        admin_h = {"Authorization": f"Bearer {r.json()['access_token']}"}

        org_id = (
            await c.post(f"{BASE}/awareness/organizations", json={"name": "LB Org"}, headers=admin_h)
        ).json()["id"]
