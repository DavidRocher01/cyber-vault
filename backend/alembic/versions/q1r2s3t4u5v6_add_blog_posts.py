"""Add blog_posts table with seed articles
...
<p>En comparaison, un audit Flash à 245 € représente une prime d'assurance. Le ROI moyen d'un audit de sécurité est estimé à <strong>10× sur 3 ans</strong> selon les études du secteur.</p>
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Surveillance continue</td><td>Abonnement Sentinelle</td><td>199 € HT/mois</td></tr>
  </tbody>
</table>
...
def upgrade() -> None:
    ...
    conn.execute(
        sa.text(
            "INSERT INTO blog_posts (slug, title, description, date, read_time, category, tags, html_content, is_published, created_at, updated_at) VALUES "
            "(:slug1, :title1, :desc1, :date1, :rt1, :cat1, :tags1, :html1, :pub1, :now, :now), "
            "(:slug2, :title2, :desc2, :date2, :rt2, :cat2, :tags2, :html2, :pub2, :now, :now)"
        ),
        {
            "slug1": "audit-cybersecurite-pme-prix-2026",
            "title1": "Audit cybersécurité PME : combien ça coûte vraiment en 2026 ?",
            "desc1": "Tarifs détaillés, types d'audits, ce qui est inclus et le ROI pour une TPE/PME. Guide complet par un développeur-auditeur basé en Auvergne-Rhône-Alpes.",
            "date1": "2026-05-01",
            "rt1": 8,
            "cat1": "Audit & Conseils",
            "tags1": json.dumps(["audit cybersécurité", "PME", "prix", "pentest", "RGPD"]),
            "html1": ARTICLE_1_HTML,
            "pub1": True,
            "slug2": "vulnerabilites-courantes-sites-ecommerce",
            "title2": "Les 10 vulnérabilités les plus courantes sur les sites e-commerce français",
            "desc2": "XSS, injection SQL, mauvaise config SSL... Les failles que l'on trouve à 90 % sur les sites e-commerce lors de nos audits. Exemples concrets et solutions.",
            "date2": "2026-05-12",
            "rt2": 10,
            "cat2": "Sécurité Web",
            "tags2": json.dumps(["e-commerce", "XSS", "injection SQL", "OWASP", "sécurité web"]),
            "html2": ARTICLE_2_HTML,
            "pub2": True,
            "now": now,
        },
    )
