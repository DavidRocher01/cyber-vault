...
async def compute_nis2_metrics(db: AsyncSession, org_id: int) -> dict:
    ...
    participation_pct = round(enrolled_learners / total_learners * 100, 1) if total_learners else 0.0
        ...
        modules = (
            await db.execute(
                select(AwarenessModule.id).where(
                    ...
                    func.lower(AwarenessModule.title).contains(keywords[0])
                )
            )
        ).scalars().all()
...
def generate_nis2_report_pdf(
    ...
    h1 = ParagraphStyle("rh1", fontSize=22, textColor=_CYAN, fontName="Helvetica-Bold",
                         spaceAfter=4)
    h2 = ParagraphStyle("rh2", fontSize=13, textColor=_CYAN, fontName="Helvetica-Bold",
    ...
    story = []
    story.append(Paragraph(f"Rapport de conformité NIS2", h1))
    ...
    story.append(Paragraph("Score de conformité global", ParagraphStyle(
        "rlabel", fontSize=9, textColor=_GRAY, alignment=1, spaceAfter=12
    )))
    ...
    for req in requirements:
        rows.append([
            ...
            f"{req['value']:.1f}%",
            f"{req['threshold']}%",
            Paragraph(req["status_label"], ParagraphStyle(
                "rs", fontSize=8,
    ...
    t.setStyle(TableStyle([
        ...
        ("FONTSIZE", (0, 0), (-1, 0), 8),
        ("ROWBACKGROUNDS", (0, 1), (-1, -1), [colors.HexColor("#0f172a"), colors.HexColor("#111c30")]),
    ...
    metrics_data = [
        ...
        ["Taux de participation des learners", f"{metrics.get('learner_participation_pct', 0):.1f}%"],
