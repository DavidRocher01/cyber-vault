    ...
    try:
        ...
        if resp.status_code == 429:
            return {"email": email, "breaches": [], "total": 0, "status": "unknown",
                    "error": "Rate limited — retry later", "provider": "leakcheck"}
        resp.raise_for_status()
        data = resp.json()
        if not data.get("success"):
            return {"email": email, "breaches": [], "total": 0, "status": "unknown",
                    "error": data.get("error", "Unknown error"), "provider": "leakcheck"}
        ...
        return {"email": email, "breaches": breaches, "total": found,
                "status": status, "error": None, "provider": "leakcheck"}
    except requests.exceptions.RequestException as exc:
        return {"email": email, "breaches": [], "total": 0, "status": "unknown",
                "error": str(exc), "provider": "leakcheck"}
...
def check_email_hibp(email: str, api_key: str) -> dict[str, Any]:
    ...
    if not api_key:
        return {"email": email, "breaches": [], "total": 0, "status": "unknown",
                "error": "HIBP API key not configured", "provider": "hibp"}
    ...
    try:
        ...
        if resp.status_code == 404:
            return {"email": email, "breaches": [], "total": 0, "status": "OK",
                    "error": None, "provider": "hibp"}
        if resp.status_code == 401:
            return {"email": email, "breaches": [], "total": 0, "status": "unknown",
                    "error": "Invalid HIBP API key", "provider": "hibp"}
        if resp.status_code == 429:
            return {"email": email, "breaches": [], "total": 0, "status": "unknown",
                    "error": "Rate limited — retry later", "provider": "hibp"}
        resp.raise_for_status()
        raw = resp.json()
        breaches = [
            {
        ...
        return {"email": email, "breaches": breaches, "total": total,
                "status": status, "error": None, "provider": "hibp"}
    except requests.exceptions.RequestException as exc:
        return {"email": email, "breaches": [], "total": 0, "status": "unknown",
                "error": str(exc), "provider": "hibp"}
...
def check_domain_breaches(domain: str, api_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        total = len(accounts)
        status = "CRITICAL" if total >= 3 else ("WARNING" if total > 0 else "OK")
        return {"domain": domain, "accounts": accounts, "total": total,
                "status": status, "error": None}
    except requests.exceptions.RequestException as exc:
        return {"domain": domain, "accounts": [], "total": 0,
