...
def _styles() -> dict:
    return {
        ...
        "subtitle": ParagraphStyle(
            "r_sub", fontName="Helvetica", fontSize=11, textColor=GRAY, spaceAfter=14
        ),
        "h2": ParagraphStyle(
            "r_h2",
            fontName="Helvetica-Bold",
        ...
        "td_amb": ParagraphStyle(
            "r_td_amb", fontName="Helvetica-Bold", fontSize=9, textColor=_AMBER
        ),
        "footer": ParagraphStyle(
            "r_footer", fontName="Helvetica", fontSize=8, textColor=GRAY, alignment=1
        ),
    }
...
def _status_style(status: str, styles: dict) -> ParagraphStyle:
    return {
        ...
        "in_progress": ParagraphStyle(
            "r_td_blue", fontName="Helvetica-Bold", fontSize=9, textColor=_BLUE
        ),
...
def generate_rssi_report(
    ...
    story.append(
        _kv_table(
            [
                ...
                (
                    "Actions terminées",
                    str(len([a for a in actions if a.get("status") == "done"])),
                ),
    ...
    if actions:
        ...
        sorted_actions = sorted(
            actions, key=lambda a: _priority_order.get(a.get("priority", ""), 9)
        )
        ...
        for a in sorted_actions:
            ...
            rows.append(
                [
                    ...
                    Paragraph(
                        _priority_fr.get(a.get("priority", ""), "—"),
                        _priority_style(a.get("priority", ""), styles),
                    ),
                    Paragraph(
                        _status_fr.get(a.get("status", ""), "—"),
                        _status_style(a.get("status", ""), styles),
                    ),
                    Paragraph(
                        _fmt_date(a.get("due_date")),
                        styles["td_red"] if is_overdue else styles["small"],
                    ),
        ...
        for v in recent:
            ...
            rows.append(
                [
                    ...
                    Paragraph(
                        f"{v['duration_hours']}h" if v.get("duration_hours") else "—",
                        styles["small"],
                    ),
                ]
            )
