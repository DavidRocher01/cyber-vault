    ...
    return [
        {
            ...
            "status": c.status,
            "contract_renewal_at": c.contract_renewal_at.isoformat()
...
async def get_pending_alerts(consultant_id: int, db: AsyncSession) -> list[dict]:
    ...
    if client_ids:
        premium_ids = [cid for cid, c in clients.items() if c.formula in ("premium", "excellence")]
        if premium_ids:
            ...
            for cid in premium_ids:
                ...
                if last is None or last < visit_horizon:
                    ...
                    c = clients[cid]
                    detail = (
                        f"Dernière visite il y a {days_ago} jour(s)"
                        if days_ago
                        else "Aucune visite enregistrée"
                    )
                    alerts.append(
                        {
