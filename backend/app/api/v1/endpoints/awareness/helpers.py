...
from app.services.awareness import access_service


async def _get_org_or_404(org_id: int, user: User, db: AsyncSession) -> AwarenessOrganization:
    org = await access_service.get_org_for_owner(db, org_id, user.id)
    if org is None:
...
async def _get_learner_or_404(learner_id: int, org_id: int, db: AsyncSession) -> AwarenessLearner:
    learner = await access_service.get_learner_in_org(db, learner_id, org_id)
    if learner is None:
