...
@Component({
  ...
  template: `
    <div style="min-height:100vh;background:#0f172a;font-family:Arial,sans-serif;">
      <!-- Navbar -->
      <nav
        style="padding:16px 24px;display:flex;align-items:center;gap:12px;border-bottom:1px solid rgba(255,255,255,0.06);"
      >
        <app-nav-buttons />
        <a
          routerLink="/"
      ...
      <div style="max-width:820px;margin:0 auto;padding:40px 24px;">
        ...
        @if (apiKeySet()) {
          ...
          >
            <div
              style="background:#1e293b;border:1px solid #334155;border-radius:12px;padding:20px;text-align:center;"
            >
              <p style="margin:0 0 4px;font-size:28px;font-weight:800;color:#f1f5f9;">
                {{ stats()?.total ?? '…' }}
              </p>
              <p
            ...
            <div
              style="background:#1e293b;border:1px solid #22d3ee33;border-radius:12px;padding:20px;text-align:center;"
            >
              <p style="margin:0 0 4px;font-size:28px;font-weight:800;color:#22d3ee;">
                {{ stats()?.active ?? '…' }}
              </p>
              <p
                style="margin:0;color:#64748b;font-size:12px;text-transform:uppercase;letter-spacing:1px;"
            ...
            <div
              style="background:#1e293b;border:1px solid #f59e0b33;border-radius:12px;padding:20px;text-align:center;"
            >
              <p style="margin:0 0 4px;font-size:28px;font-weight:800;color:#f59e0b;">
          ...
          <!-- Planning — 6 slots -->
          <div
            style="background:#1e293b;border:1px solid #334155;border-radius:12px;padding:32px;margin-bottom:24px;"
          >
            <div
            ...
            >
              <div>
                ...
                <p style="color:#64748b;font-size:13px;margin:0;">
                  Remplissez les articles puis enregistrez.
                </p>
              </div>
              <button
                (click)="saveSchedule()"
            ...
            @if (saveOk()) {
              <div
                style="background:rgba(34,197,94,0.1);border:1px solid rgba(34,197,94,0.3);border-radius:8px;padding:12px 16px;margin-bottom:20px;display:flex;align-items:center;gap:8px;"
              >
                <mat-icon style="color:#4ade80;font-size:18px;width:18px;height:18px;"
                ...
                @for (ctrl of articleControls; track $index) {
                  ...
                  >
                    ...
                    <div style="display:flex;align-items:center;gap:10px;margin-bottom:16px;">
                      <span
                        [style.background]="accent($index) + '22'"
                    ...
                    >
                      <div>
                        <label
                          style="display:block;color:#475569;font-size:11px;font-weight:600;letter-spacing:1px;margin-bottom:4px;"
                          >SOURCE</label
                        >
                        <input
                          formControlName="actu_source"
                      ...
                      <div>
                        <label
                          style="display:block;color:#475569;font-size:11px;font-weight:600;letter-spacing:1px;margin-bottom:4px;"
                          >URL IMAGE <span style="color:#334155;">(optionnel)</span></label
                        >
                        <input
                    ...
                    <div style="margin-bottom:10px;">
                      ...
                      >
                      <input
                        formControlName="actu_title"
                        placeholder="Ex: Campagne ransomware vise les PME européennes"
                        style="width:100%;box-sizing:border-box;background:#1e293b;border:1px solid #334155;border-radius:6px;padding:9px 12px;color:#f1f5f9;font-size:13px;outline:none;"
                      />
                    </div>
                    ...
                    <div style="margin-bottom:10px;">
                      <label
                        style="display:block;color:#475569;font-size:11px;font-weight:600;letter-spacing:1px;margin-bottom:4px;"
                        >URL DE L'ARTICLE</label
                      >
                      <input
                        formControlName="actu_url"
                        placeholder="https://..."
                    ...
                    <div>
                      <label
                        style="display:block;color:#475569;font-size:11px;font-weight:600;letter-spacing:1px;margin-bottom:4px;"
                        >NOTE (pourquoi lire cet article ?)</label
                      >
                      <input
                        formControlName="reflex"
                        placeholder="Ex: Réduire le délai de détection grâce à un EDR/SIEM"
                        style="width:100%;box-sizing:border-box;background:#1e293b;border:1px solid #334155;border-radius:6px;padding:9px 12px;color:#f1f5f9;font-size:13px;outline:none;"
                      />
                    </div>
                  </div>
          ...
          <div style="background:#1e293b;border:1px solid #334155;border-radius:12px;padding:32px;">
            ...
            <p style="color:#64748b;font-size:13px;margin:0 0 20px;">
              Sera envoyée aux
              <strong style="color:#22d3ee;">{{ stats()?.active ?? '?' }} abonnés actifs</strong>.
            </p>
            <div style="display:flex;gap:12px;align-items:flex-end;flex-wrap:wrap;">
              <div>
                <label
                  style="display:block;color:#475569;font-size:11px;font-weight:600;letter-spacing:1px;margin-bottom:6px;"
                  >NUMÉRO D'ÉDITION</label
                >
                <input
                  type="number"
                  [(ngModel)]="editionNumber"
              ...
              >
                @if (sending()) {
                  ...
                  Envoyer l'édition #{{ editionNumber }}
                }
              </button>
            </div>
            ...
            @if (sendResult()) {
              <div
                [style.background]="
                  sendResult()!.ok ? 'rgba(34,197,94,0.1)' : 'rgba(239,68,68,0.1)'
                "
                [style.border]="
                  sendResult()!.ok
              ...
              >
                ...
                <span
                  [style.color]="sendResult()!.ok ? '#4ade80' : '#f87171'"
...
export class NewsletterAdminComponent implements OnInit {
  ...
  fetchOgImage(index: number) {
    ...
    this.http
      ...
      .subscribe({
        next: res => {
          if (res.image_url) ctrl.patchValue({ image_url: res.image_url });
        },
      });
  }
  ...
  sendFromSchedule() {
    ...
    this.http
      ...
      .subscribe({
        ...
        error: () => {
          this.sending.set(false);
          this.sendResult.set({ ok: false, message: "Erreur lors de l'envoi." });
        },
      });
  }
