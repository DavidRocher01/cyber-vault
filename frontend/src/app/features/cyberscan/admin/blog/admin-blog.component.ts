...
export class AdminBlogComponent implements OnInit {
    ...
    this.http.get<BlogPost>(`/api/v1/blog/articles/${article.slug}`, { headers: this.auth.headers() }).subscribe({
      next: a => {
        this.form.setValue({
          ...
          date: a.date, readTime: a.readTime, category: a.category,
          tags: a.tags.join(', '), htmlContent: a.htmlContent ?? '',
  ...
  save() {
    ...
    const payload = {
      ...
      date: v.date!, readTime: v.readTime!, category: v.category!,
    ...
    req.subscribe({
      ...
      error: err => { this.saveError.set(err?.error?.detail ?? 'Erreur'); this.saving.set(false); },
    });
  }
