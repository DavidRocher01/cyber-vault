...
describe('ApiLandingComponent — colorFor()', () => {
  ...
  it('text-purple-400 pour purple', () => expect(make().colorFor('purple')).toBe('text-purple-400'));
  it('text-indigo-400 pour indigo', () => expect(make().colorFor('indigo')).toBe('text-indigo-400'));
  it('fallback text-cyan-400 pour couleur inconnue', () => expect(make().colorFor('red')).toBe('text-cyan-400'));
});
...
describe('ApiLandingComponent — borderFor()', () => {
  ...
  it('fallback cyan pour couleur inconnue', () => expect(make().borderFor('orange')).toContain('cyan'));
});
