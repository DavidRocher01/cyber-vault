...
export class UrlScannerComponent implements OnInit, OnDestroy {
  ...
  submit() {
    ...
    this.cyberscan.triggerUrlScan(this.form.getRawValue().url).subscribe({
      next: scan => {
        ...
        this.snack.open('Analyse lancée — résultat dans quelques secondes', 'OK', {
          duration: 5000,
        });
  ...
  verdictBg(verdict: string | null): string {
    switch (verdict) {
      case 'safe':
        return 'bg-green-400/10 border-green-700 text-green-400';
      case 'suspicious':
        return 'bg-yellow-400/10 border-yellow-700 text-yellow-400';
      case 'malicious':
        return 'bg-red-400/10 border-red-700 text-red-400';
      default:
