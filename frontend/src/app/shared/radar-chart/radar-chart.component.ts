    ...
    <svg viewBox="0 0 300 300" class="w-full h-full">
      ...
      @for (axis of axes; track axis.label) {
        <line
          [attr.x1]="cx"
          [attr.y1]="cy"
          [attr.x2]="axis.lineEnd.x"
          [attr.y2]="axis.lineEnd.y"
          stroke="#374151"
          stroke-width="1"
          opacity="0.6"
        />
      ...
      <!-- Data fill -->
      <polygon [attr.points]="dataPolygon" fill="rgba(6,182,212,0.12)" stroke="none" />
      ...
      @for (axis of axes; track axis.label) {
        ...
        <!-- Score value -->
        <text
          [attr.x]="axis.dataPoint.x + axis.scoreOffset.x"
          [attr.y]="axis.dataPoint.y + axis.scoreOffset.y"
          text-anchor="middle"
          dominant-baseline="middle"
          font-size="8"
          font-weight="bold"
          [attr.fill]="scoreColor(axis.score)"
        >
          {{ axis.score }}
        </text>
      }
