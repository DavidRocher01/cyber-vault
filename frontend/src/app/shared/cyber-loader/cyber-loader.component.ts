...
const LOG_POOL: LogLine[] = [
  {
    lvl: 'INF',
    msg: 'TLS 1.3 handshake complete with edge-07.fr3 — cipher TLS_AES_256_GCM_SHA384',
  },
    ...
    `
      ...
      .phos-glow {
        text-shadow:
          ...
          0 0 14px oklch(0.8 0.2 145 / 0.45);
      }
      .topbar {
        display: flex;
...
export class CyberLoaderComponent implements OnInit, OnDestroy {
  ...
  get sessionId(): string {
    return hexBlock(12, Math.floor(this.tick / 40) + 1);
  }
  get hash(): string {
    return hexBlock(64, Math.floor(this.tick / 4) + 1);
  }
  get pctStr(): string {
    return String(Math.floor(this.loopPct * 100)).padStart(3, '0');
  }
