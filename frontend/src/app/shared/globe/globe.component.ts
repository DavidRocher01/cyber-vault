  ...
  private boot() {
    ...
    const draw = (ts: number) => {
      ...
      for (let i = impacts.length - 1; i >= 0; i--) {
        ...
        ctx.beginPath();
        ctx.arc(imp.x, imp.y, 10, 0, Math.PI * 2);
        ctx.fillStyle = flash;
        ctx.fill();
      }
      ...
      for (const city of CITIES) {
        ...
        ctx.beginPath();
        ctx.arc(p.x, p.y, haloSz, 0, Math.PI * 2);
        ctx.fillStyle = h1;
        ctx.fill();
        ...
        ctx.fillStyle = `rgba(255,255,255,${depth * 0.95})`;
        ctx.fill();
      }
      ...
      ctx.fillText(
        `● ${Math.floor(this.attackCount * 0.62).toLocaleString('fr-FR')} TENTATIVES RÉSEAU`,
        W - 18,
        H - 16
      );
    };
