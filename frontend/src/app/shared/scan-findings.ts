...
export function extractSummary(
  ...
  switch (key) {
    case 'ssl':
      return [
        d['days_remaining'] != null
          ? { label: 'Expire dans', value: `${d['days_remaining']} jours` }
          : null,
    ...
    case 'cookies': {
      ...
      const labels =
        issues
          ?.map(i => (typeof i === 'string' ? i : (i.issue ?? i.cookie ?? '')))
          .filter(Boolean)
          .slice(0, 3) ?? [];
    ...
    case 'ip':
      return [
        ...
        d['total_listed'] != null
          ? { label: 'Blacklists', value: `${d['total_listed']} liste(s)` }
          : null,
        d['listed_in'] && (d['listed_in'] as string[]).length
          ? { label: 'Listé dans', value: (d['listed_in'] as string[]).slice(0, 2).join(', ') }
          : null,
      ].filter(Boolean) as { label: string; value: string }[];
    ...
    case 'tech': {
      ...
      return techs?.length ? [{ label: 'Technologies', value: techs.slice(0, 5).join(', ') }] : [];
    }
    ...
    case 'robots': {
      ...
      return [
        sensitive?.length
          ? { label: 'Chemins sensibles', value: sensitive.slice(0, 3).join(', ') }
          : { label: 'Chemins sensibles', value: 'Aucun' },
...
const EXPLANATIONS: Record<
  string,
  Partial<Record<'OK' | 'WARNING' | 'CRITICAL' | 'default', string>>
> = {
  ssl: {
    ...
    WARNING:
      "Votre certificat arrive bientôt à expiration ou utilise un protocole vieillissant. Sans renouvellement, les navigateurs afficheront une alerte de sécurité et bloqueront l'accès à votre site.",
    CRITICAL:
      "Votre certificat SSL/TLS est invalide ou expiré. Les navigateurs bloquent l'accès à votre site et vos visiteurs voient une page d'erreur. Action urgente requise.",
  },
  headers: {
    ...
    WARNING:
      'Certains en-têtes de sécurité HTTP sont manquants. Ces en-têtes sont des lignes de défense côté navigateur — leur absence facilite certaines attaques.',
    CRITICAL:
      'Des en-têtes de sécurité critiques sont absents. Votre site est vulnérable à des attaques comme le XSS, le clickjacking ou le vol de session.',
  },
  email: {
    ...
    WARNING:
      'Certains enregistrements de sécurité email sont manquants ou mal configurés. Des attaquants pourraient usurper votre domaine pour envoyer des emails frauduleux à vos clients.',
    CRITICAL:
      "Vos protections email sont absentes. N'importe qui peut envoyer des emails en se faisant passer pour vous — risque élevé de phishing ciblant vos clients.",
  },
  cookies: {
    ...
    WARNING:
      'Certains cookies manquent de flags de sécurité. Un attaquant pourrait potentiellement les voler via du JavaScript malveillant ou les intercepter.',
    CRITICAL:
      'Des cookies de session critiques sont exposés sans protection. Un vol de session (session hijacking) est facilement réalisable.',
  },
  cors: {
    ...
    WARNING:
      'Votre configuration CORS est trop permissive. Des sites tiers pourraient faire des requêtes à votre API au nom de vos utilisateurs connectés.',
    CRITICAL:
      "CORS ouvert à tous (`*`). N'importe quel site web peut interroger votre API en se faisant passer pour un utilisateur authentifié.",
  },
  ip: {
    ...
    WARNING:
      'Votre IP est présente sur une ou plusieurs listes noires. Cela peut affecter la délivrabilité de vos emails et nuire à la confiance envers votre domaine.',
    CRITICAL:
      'Votre IP est blacklistée sur plusieurs listes. Vos emails sont probablement bloqués et votre site peut être filtré par certains services de sécurité.',
  },
  dns: {
    OK: "Aucun sous-domaine sensible n'a été détecté en dehors de ceux attendus. Votre surface d'attaque DNS est maîtrisée.",
    WARNING:
      'Des sous-domaines ont été trouvés — certains pourraient pointer vers des services abandonnés. Un sous-domaine non maintenu peut être réutilisé par un attaquant (subdomain takeover).',
    CRITICAL:
      'Des sous-domaines sensibles sont exposés et potentiellement réutilisables. Le risque de subdomain takeover est élevé.',
  },
  cms: {
    OK: "Aucun CMS détecté en dehors de ce qui est attendu. La surface d'attaque liée aux CMS est minimisée.",
    WARNING:
      "Un CMS a été détecté. Assurez-vous qu'il est à jour — les CMS populaires sont des cibles fréquentes d'attaques automatisées.",
    CRITICAL:
      'Un CMS obsolète ou mal configuré a été détecté. Des failles connues peuvent être exploitées automatiquement par des scanners malveillants.',
  },
  waf: {
    ...
    WARNING:
      'Aucun WAF détecté. Sans pare-feu applicatif, les attaques courantes (SQLi, XSS, scans automatisés) arrivent directement à votre application.',
    CRITICAL:
      'Aucun WAF détecté et des vulnérabilités critiques ont été identifiées. Votre application est exposée sans filtrage.',
  },
  tech: {
    ...
    WARNING:
      'Des technologies et leurs versions sont exposées dans vos en-têtes. Cela permet à un attaquant de rechercher des CVE ciblant vos versions exactes.',
    CRITICAL:
      'Des technologies vulnérables ont été identifiées et leurs versions sont exposées. Des exploits publics existent potentiellement pour ces versions.',
  },
  tls: {
    ...
    WARNING:
      'Des protocoles TLS faibles (TLS 1.0/1.1) sont encore supportés. Un attaquant peut forcer une connexion vers un protocole moins sécurisé (downgrade attack).',
    CRITICAL:
      'Votre configuration TLS est dangereuse. Des protocoles obsolètes sont actifs et/ou HSTS est absent — les connexions peuvent être interceptées.',
  },
  takeover: {
    ...
    WARNING:
      "Des sous-domaines suspects ont été détectés. Un subdomain takeover permet à un attaquant de prendre le contrôle d'un sous-domaine de votre marque.",
    CRITICAL:
      'Un ou plusieurs sous-domaines sont vulnérables au takeover. Un attaquant peut héberger du contenu malveillant sous votre domaine.',
  },
  threat_intel: {
    OK: "Aucune menace connue associée à votre infrastructure. Votre IP n'est pas référencée dans les bases de menaces actives.",
    WARNING:
      "Des ports ouverts ou des informations de menace ont été détectés. Chaque port ouvert inutile est une surface d'attaque supplémentaire.",
    CRITICAL:
      'Des CVEs connues et/ou des ports à haut risque ont été détectés sur votre infrastructure. Une exploitation active est possible.',
  },
  http_methods: {
    OK: 'Seules les méthodes HTTP nécessaires sont autorisées. Les méthodes dangereuses (PUT, DELETE, TRACE) sont désactivées.',
    WARNING:
      'Des méthodes HTTP potentiellement dangereuses sont activées. TRACE peut faciliter des attaques XST (Cross-Site Tracing).',
    CRITICAL:
      'Des méthodes HTTP dangereuses sont actives (PUT, DELETE). Un attaquant peut modifier ou supprimer des ressources sur votre serveur.',
  },
  open_redirect: {
    ...
    WARNING:
      "Des redirections potentiellement manipulables ont été détectées. Un attaquant peut créer des liens d'apparence légitime qui redirigent vers du phishing.",
    CRITICAL:
      'Des redirections ouvertes exploitables ont été confirmées. Votre domaine peut être utilisé activement dans des campagnes de phishing.',
  },
  clickjacking: {
    ...
    WARNING:
      'La protection anti-clickjacking est partielle ou manquante. Un attaquant peut superposer votre page sous une interface trompeuse.',
    CRITICAL:
      "Aucune protection contre le clickjacking. Votre site peut être intégré dans n'importe quelle page pour piéger vos utilisateurs en leur faisant effectuer des actions à leur insu.",
  },
  directory_listing: {
    OK: 'Le listing de répertoires est désactivé. Les fichiers de votre serveur ne sont pas listables publiquement.',
    WARNING:
      'Le listing de répertoires est potentiellement actif sur certains chemins. Des fichiers de configuration ou sensibles pourraient être exposés.',
    CRITICAL:
      "Le listing de répertoires est activé. N'importe qui peut parcourir l'arborescence de vos fichiers serveur — configs, logs, backups potentiellement exposés.",
  },
  robots: {
    ...
    WARNING:
      'Votre fichier robots.txt mentionne des chemins sensibles dans les règles `Disallow`. Paradoxalement, cela indique à un attaquant où chercher des pages cachées.',
    CRITICAL:
      "Des chemins d'administration ou sensibles sont exposés dans robots.txt. C'est une liste de cibles potentielles pour les attaquants.",
  },
  jwt: {
    OK: 'Aucun token JWT exposé ou mal configuré détecté sur vos pages publiques.',
    WARNING:
      'Des problèmes de configuration JWT ont été détectés. Des tokens mal signés ou avec une durée de vie excessive peuvent être réutilisés par un attaquant.',
    CRITICAL:
      "Des tokens JWT présentent des failles critiques (algorithme `none`, clé faible). Un attaquant peut forger des tokens et usurper l'identité de vos utilisateurs.",
  },
};
...
export function getFindings(resultsJson: string | null): Finding[] {
  ...
  return MODULE_META.map(m => {
    ...
    const status = skipped
      ? null
      : ((data['status'] as 'OK' | 'WARNING' | 'CRITICAL' | null) ?? null);
    return {
      ...m,
      skipped,
      status,
