...
- **Lot 2 — Cibles non destructives. ✅ FAIT.** `upload_targets_csv` en **merge/dédup** par défaut (n'écrase plus ; `?replace=true` pour l'ancien comportement), endpoints d'**ajout** (`POST /targets/single`, dédup → 409) et **suppression** (`DELETE /targets/{id}`) unitaires, `targets_count` resynchronisé (`_recount_targets`). UI d'édition : liste des cibles avec suppression + ajout par email + feedback d'import (ajoutées/ignorées). (Listes réutilisables = follow-up.)
...
- **Lot 5 — Look-alike à deux niveaux** (découpé en 5a/5b).
  - **5a — Fix landing cross-host. ✅ FAIT.** `get_landing_html(base=...)` + l'endpoint `/l` passe `_tracking_base(campaign)` → le formulaire de la landing poste sur le MÊME host que celui qui l'a servie (le look-alike si présent), plus sur `PHISHING_BASE_URL` en dur.
  - **5b — Domaine acheté (lourd).** `sending_mode`, UI sous-domaine vs domaine acheté, suggestions (`generate_lookalikes`), flux vérif DNS-TXT + Resend dans l'UI, case de mandat (traçée), gating premium.
