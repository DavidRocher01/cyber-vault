...
- **Lot 0 — Fondations & nettoyage.** Enum de statut unifié, PDF UI réparé, code mort retiré (`reported`, `PHISHING_FROM_NAME`), `.env.example` corrigé, SSRF guard sur `lookalike_domain`. *Risque faible, débloque le reste.*
- **Lot 1 — Modèle deux opérateurs + gating.** `rssi_client_id` + migration, `require_phishing_access`, scoping consultant, onglet « Phishing » dans la fiche client RSSI.
- **Lot 2 — Cibles non destructives.** Merge/dédup CSV, ajout/suppression unitaire, listes réutilisables.
- **Lot 3 — Planif + annulation + cadence.** Bouton annuler (`cancelled` câblé), cadence par campagne.
- **Lot 4 — Training-on-fail.** Câblage `record_click`/`submit` → enrôlement awareness (org client/entreprise), toggle par campagne.
