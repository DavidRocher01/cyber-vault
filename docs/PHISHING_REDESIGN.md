...
- **Lot 1 — Modèle deux opérateurs + gating. ✅ FAIT.** `rssi_client_id` (nullable FK, migration `97647476457e`), gating à la création (entreprise directe → tier ≥ 3 Pro ; consultant → ownership du client, 404 sinon), filtre de liste (`?rssi_client_id=` vs entreprise-only), onglet « Phishing » dans la fiche client RSSI (liste + « Nouvelle campagne » → config). Ownership = `user_id` (le consultant crée sous son compte) ; `rssi_client_id` = attribution.
